#' ---
#' title: "13: Extract unrestricted community model omegas and MCMC draws"
#' author: "Jamie Crowther"
#' date: '`r format(Sys.Date(), "%B %d %Y")`'
#' output: html_document
#' ---

# Extract omega summary from models and save as one dataframe 
...
m_df <- m_df %>% mutate(repo = "Community")

# Save 
write_csv(m_df, "Outputs/VM_unrestricted_community_omega_summaries.csv")
...
for (i in seq_along(fits)) {
  ...
  # Get common comorbidities for index condition
  ...
  # Bind rows
  om_draws_df <- bind_rows(om_draws_df, om_draws_long)
}

# Add identifying column 
om_draws_df <- om_draws_df %>% mutate(repo = "Community")

# Save 
saveRDS(om_draws_df, "Outputs/VM_unrestricted_community_omega_draws.RDS")
