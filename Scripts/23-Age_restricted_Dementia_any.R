#' ---
#' title: "23: Age restricted IPD dementia"
...
condition <- unique(dem$index_condition)
condition <- condition %>% str_replace_all(" ", "_") %>% str_remove_all("\\(|\\)")

run_mod2(run_info = paste0("0309-Age_restricted_IPD_", condition), 
        data_list = dem_data_list, 
        stanmodel_file = "Scripts/Multivariate_probit_model.stan",
        save_path = "Outputs/age_resctricted_model_fits/")
