...
list.files("Supporting/Prepare_trial_data/GSK/Created_metadata/", patt = "search") %>%  dput()

slctd <- map(c("ckd_epi_search.csv", "egfr_search.csv", "mdrd_search.csv"), ~
               read_csv(paste0("Supporting/Prepare_trial_data/GSK/Created_metadata/", .x)))

names(slctd) <- c("ckd_epi", "egfr", "mdrd")
slctd <- bind_rows(slctd, .id = "searched_on")
slctd <- slctd %>% arrange(document)
...
# 18 without egfr, but with creatinine
...
race_lkp <- read_csv("Supporting/Prepare_trial_data/GSK/Created_metadata/for_egfr_race.csv")
...
aa <- read_csv("Supporting/Prepare_trial_data/GSK/Created_metadata/terms_used_aa.csv")
...
## SAVE
saveRDS(egfr, "GSK_processed_data/egfr.Rds")
