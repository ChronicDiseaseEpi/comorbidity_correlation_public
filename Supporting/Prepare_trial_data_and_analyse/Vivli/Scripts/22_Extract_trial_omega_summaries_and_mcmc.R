# Extract omega summary (only 1 corner of matrix) for each trial and combine 
# into a single dataframe. Code further down extracts mcmc draws for omega and
# combines them into one big df for all trials. 

source("Supporting/Prepare_trial_data/Vivli/Scripts/00_functions_and_packages.R")

fit_paths  <- list.files("Results/model_fits/", full.names = TRUE)[-37]
vivli_como <- read.csv("Processed_data/cleaned_conmed_defined_comorbidity.csv", row.names = 1)
cmnty_como <- read.csv("Supporting/Prepare_trial_data/Vivli/Created_metadata/SAIL_common_comorbidity_proportion_harmonised.csv", row.names = 1)
...
# Get trial from model fit file path 
trial_fit_path  <- fit_paths[i]
tr              <- trials[str_detect(trial_fit_path, trials)]

# Get trial IPD
trial_data <- vivli_como %>% filter(trial %in% tr)

# Get trial index condition 
...
# Melt
omega  <- as.data.frame(as.table(omega)) %>% rename(mean = Freq)
ci2.5  <- as.data.frame(as.table(ci2.5)) %>% rename(ci2.5 = Freq)
ci97.5 <- as.data.frame(as.table(ci97.5)) %>% rename(ci97.5 = Freq)
sd     <- as.data.frame(as.table(sd)) %>% rename(sd = Freq)
n_eff  <- as.data.frame(as.table(n_eff)) %>% rename(n_eff = Freq)
rhat   <- as.data.frame(as.table(rhat)) %>% rename(rhat = Freq)
...
# Add ID column
m_df <- m_df %>% mutate(repo = "Vivli")

# Save 
write_csv(m_df, "Supporting/Prepare_trial_data/Vivli/Results/vivli_omega_summaries.csv")
...
for (i in seq_along(fit_paths)) {
  ...
  # Get trial data then nct id and condition  
  trial_data <- vivli_como %>% filter(trial %in% trial_name)
  trial_id <- unique(trial_data$nct_id)
  trial_condition <- unique(trial_data$condition)
  
  # Get 6 most common comorbidities in community IPD for trial index condition
  trial_common_como <- cmnty_como %>%
    filter(condition %in% trial_condition) %>%
    distinct(como) %>% 
    unlist() %>% 
    as.character() 
...
# Add in id column 
om_draws_df <- om_draws_df %>% mutate(repo = "Vivli")

# Save 
saveRDS(om_draws_df, "Supporting/Prepare_trial_data/Vivli/Results/vivli_omega_draws.RDS")
