...
lilly <- readRDS("Processed_data/lilly_conmed.Rds")



## Consolidate drugs ----
## Extract conmed table and lookup table from RXNORM and BNF and table of ATC classes
conmed <- cmpnies$conmed
rm(cmpnies)

lilly <- lilly %>% 
  rename(atc_code = atccd4, term = cmterm, route = c_route) %>% 
  mutate(company = "lilly")

drg_lkp <- read_csv("V:/2022_05_12 Scripts for Analysis/bnf_rxnorm_atc_codes.csv")
class_names <- read_csv("V:/2022_05_12 Scripts for Analysis/class_names.csv")
...
write_csv(route, "Scratch_data/review_route_values.csv")
route_lkp <- read_csv("Created_metadata/reviewED_route_values.csv")

## Pasted not yet classified lilly terms into route lookup table
lilly_route <- lilly %>% 
  distinct(route) %>% 
  mutate(route= str_to_lower(route)) %>% 
  distinct(route) %>% 
  anti_join(route_lkp)
# write_csv(lilly_route, "clipboard")
setdiff(str_to_lower(lilly$route), route_lkp$route)
...
con_pre_bind <- conmed
a <- con_pre_bind %>% count(trial)

############# Add lilly data into conmed
conmed <- bind_rows(conmed, lilly)

con_post_bind <- conmed
b <- con_post_bind %>% count(trial)
...
## For some it coudl affect interpretation, so  I assigned these to the msot appropriate
...
same_term_multi_route_unq <- same_term_multi_route_unq %>% 
  separate(route_classify, into = "route1", sep = ",", remove = FALSE, extra = "drop") %>% 
  mutate(route_classify_new = if_else(selection == "take_any", route1, selection)) %>% 
  select(-selection, -route1)


# Fix sep argument with David 12/10/22 then run all subsequnet scripts down stream 
...
# Add NCT trial IDs
trial_nctids <- read.csv("Outputs/vivli_extracted_trial_nct_ID.csv", row.names = 1)

conmed <- conmed %>% inner_join(trial_nctids) 
# was only 46 trials, missing 5 trials, not 44 as removed 2 roche trials


## SAVE 
saveRDS(conmed, "Processed_data/all_sponsors_conmed.Rds")
