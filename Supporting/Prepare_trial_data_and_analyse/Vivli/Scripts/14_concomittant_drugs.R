...
rxnorm_bnf <- read_csv("v:/2022_05_12 Scripts for Analysis/bnf_rxnorm_atc_codes.csv")
...
# Trial IDs and condition
trial_indic_drug <- read_csv("v:/2022_05_12 Scripts for Analysis/selected_trials_in_respository_sas_with_medicine_condition.csv")
vivli_trials <- read.csv("../Extract_Data/Outputs/vivli_extracted_trial_nct_ID.csv", row.names = 1)

# Identify indication and medicine, all conditions have a code,
# except one Lilly file which does not belong
rand %>% 
  distinct(company, nct_id) %>% 
  anti_join(trial_indic_drug)

trial_indic_drug <- trial_indic_drug %>% 
  inner_join(vivli_trials) %>%
  select(-sponsor_id_sas, -sponsor_id)
...
rev(a)
# 7      5      4      3      1 
# 178083 243465   4347   2756     53  (28/07/22) 
rev(100*a %>%  prop.table()) %>%  cumsum() %>% round(1)
# 7     5     4     3     1 
# 41.5  98.3  99.3 100.0 100.0 (28/07/22)
sum(is.na(conmed$atc_code)) # 0 (28/07/22)
...
# There are  29712 (28/07/22) participants with one or more concomittant drug, now 29709 (17/10/22)
map_int(conmed_all, nrow) %>% unique()
# all are  29712 (28/07/22) size dataframes, now 29709 (17/10/22)
...
# Important step, stops over counting comos
conmed_all2 <- demo %>% 
...
## Save processed data
saveRDS(conmed_all, "../Extract_Data/Scratch_data/conmed_defined_comorbidity.Rds")




## Examine numbers for irregular definitions ----
conmed_all <- readRDS("../Extract_Data/Scratch_data/conmed_defined_comorbidity.Rds")
...
write.csv(conmed_smry, "../Extract_Data/Outputs/comorbidities_overall_before_removing_index.csv")
...
# Returns (28/07/22):
# "Diabetes Mellitus, Type 2, nephropathy"
# "Hypertension; Diabetic Nephropathies"

# condition not allocated any matching pairs
condition_match[condition_match == names(condition_match)] # should be empty and is empty (28/07/22)
...
conmed_all_final <- readRDS("../Extract_Data/Processed_data/conmed_defined_comorbidity.Rds")
conmed_all <- readRDS("../Extract_Data/Scratch_data/conmed_defined_comorbidity.Rds")
